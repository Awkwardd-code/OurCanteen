...
const SpecialOffersTable = () => {
    ...
    const [comboOffers, setComboOffers] = useState<SpecialOffer[]>([]);
    ...
    const [selectedCombo, setSelectedCombo] = useState<SpecialOffer | null>(null);
    ...
    const fetchCombos = async () => {
        try {
            setLoading(true);
            const data = await fetchAPI(`/(api)/special_offer`);
            setComboOffers(data);
        } catch (error) {
    ...
    const handleSaveCombo = async () => {
        if (!form.name || !form.type || !form.cuisine_id || !imageUri || !form.discount) {
        ...
        try {
            if (editMode && selectedCombo) {
                await fetchAPI("/(api)/special_offer", {
                    method: "PUT",
                    body: JSON.stringify({
                        id: selectedCombo.id,
                        name: form.name,
    ...
    const handleEditCombo = (combo: SpecialOffer) => {
        setForm({
            name: combo.name,
            type: combo.type,
    ...
    return (
        <View className="bg-white p-5 rounded-2xl border border-gray-100 shadow-lg mb-6">
            <View className="flex-row justify-between items-center mb-5">
                <Text className="text-2xl font-bold text-gray-800">Special Offers</Text>
                <TouchableOpacity
                    onPress={() => {
                        setEditMode(false);
                        resetForm();
                ...
                >
                    ...
                    <Text className="text-white font-semibold text-base">Add Special </Text>
                </TouchableOpacity>
            </View>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ minWidth: 950 }}>
                <View>
                    <View className="flex-row bg-gray-50 border-b border-gray-200 py-4 px-2">
                        ...
                        <Text className="w-60 font-semibold text-gray-600 text-sm">Discount</Text>
                    ...
                    {comboOffers.slice(0, visibleCombos).map((combo) => (
                        <View key={combo.id} className="flex-row border-b border-gray-100 py-4 px-2 items-center hover:bg-gray-50">
                            ...
                            <View className="w-60">
                                <Text className="text-sm text-gray-700 font-medium">{combo.discount}%</Text>
                            </View>
                ...
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
                    <ScrollView className="bg-white p-6 rounded-2xl max-h-[90%] shadow-2xl">
                        <Text className="text-xl font-bold text-gray-800 mb-5">
                            {editMode ? 'Edit Special Offer' : 'Add Special Offer'}
                        </Text>

                        <InputField
                            label="Special Name"
                            placeholder="Enter Special Offer name"
                            value={form.name}
