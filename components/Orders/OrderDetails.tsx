...
// Define dimensions outside component to prevent re-computation
const { width } = Dimensions.get('window');
const HORIZONTAL_MARGIN = 16;
const ITEM_WIDTH = width - HORIZONTAL_MARGIN * 2;

// Align with Orders component's Order interface
export interface Order {
  id: number;
  product_name: string;
  restaurant_name: string;
  price: number;
  is_paid: boolean; // Used to derive status
  created_at: string;
  image: string | null;
  number: number;
  amount: number;
...
const OrderDetails: React.FC<Props> = ({ item }) => {
  ...
  // Derive status from is_paid and other logic
